  ...
  smtpTransport.sendMail(mailOptions, function(error, response) {
    ...
    corpus_management.changeUsersPassword(
      ...
      function(res) {
        ...
        console.log(new Date() + " this is the user after changing their couch password " + util.inspect(user));
...
findByEmail = function(email, optionallyRestrictToIncorrectLoginAttempts, done) {
  ...
  usersdb.view("users", usersQuery, function(error, body) {
    if (error) {
      ...
      if (users.length === 0) {
        userFriendlyExplaination = "Sorry, there are no users who have failed to login who have the email you provided " + email + ". You cannot request a temporary password.";
        return done({
            status: 401,
