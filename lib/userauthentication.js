  ...
  findByUsername(req.body.username, function(err, user) {
    if (user) {
      ...
      if (dontCreateDBsForLearnXUsersBecauseThereAreTooManyOfThem &&
        (dbConnection.pouchname.indexOf("anonymouskartulispeechrecognition") > -1 ||
          dbConnection.pouchname.indexOf(/anonymous[0-9]/) > -1)) {
...
module.exports.authenticateUser = function(username, password, req, done) {
  if (!username) {
    return done({
      error: 'Username was not unspecified.' + username
    }, null, {
  ...
  if (!password) {
    return done({
      error: 'Password was not unspecified.' + password
    }, null, {
  ...
  if (username !== safeUsernameForCouchDB) {
    return done({
      error: 'The username ' + username + ' was not a real user, and potentially trying ot do somethign dangerous. ' + safeUsernameForCouchDB
    }, null, {
      message: 'Username or password is invalid. Please try again.'
    });
  }
