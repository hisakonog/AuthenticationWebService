...
module.exports.registerNewUser = function(localOrNot, req, done) {
  ...
  findByUsername(req.body.username, function(err, user) {
    if (user) {
      ...
      var corpus_template = corpus.createCorpusDoc(req.body.username, req.body.username + "\'s Corpus", couchConnection, req.body.username + "-firstcorpus");
      ...
      var datalist_template = corpus.createDatalistDoc(req.body.username + "-firstcorpus");
      ...
      var session_template = corpus.createSessionDoc(req.body.username + "-firstcorpus");
      ...
      if (result) {
        ...
        userroles.forEach(function(role) {
          if (role.indexOf(dbConn.pouchname + "_" + admin) > -1) {
            ...
            console.log(new Date() + " User " + requestingUser + " is admin and can modify permissions on " + dbConn.pouchname + " : " + util.inspect(role));
            ...
            console.log(new Date() + " User " + requestingUser + " is not admin on " + dbConn.pouchname + " : " + util.inspect(role));
          }
        });
...
module.exports.fetchCorpusPermissions = function(req, done) {
  ...
  console.log(new Date() + " Looking for corpus permissions on: " + dbConn.protocol + dbConn.domain + ':' + dbConn.port + dbConn.path);
  var nanoforpermissions = require('nano')(dbConn.protocol + couch_keys.username + ':' + couch_keys.password + '@' + dbConn.domain + ':' + dbConn.port + dbConn.path);
...
var saveUpdateUserToDatabase = function(user, done) {
  ...
  usersdb.insert(user, user.username, function(error, resultuser) {
    if (error) {
      ...
      console.log(new Date() + " This is the user who was not saved: " + util.inspect(resultuser));
      return done(error, null, {
...
var setPassword = function(oldpassword, newpassword, user, done) {
