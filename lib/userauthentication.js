var util = require('util'),
  ...
  corpus_default_template = require('../templates/corpus_template'),
...
var authServerVersion = "1.64";

console.log(new Date() + " Loading the User Authentication Module");
var nano = require('nano')(node_config.usersDbConnection.protocol + couch_keys.username + ':' + couch_keys.password + '@' + node_config.usersDbConnection.domain + ':' + node_config.usersDbConnection.port + node_config.usersDbConnection.path);
...
if (emailWhenServerStarts !== "") {
  ...
  console.log("Didn't email the devs: The FieldDB server has restarted. (It might have crashed)");
}
...
module.exports.registerNewUser = function(localOrNot, req, done) {
  ...
  findByUsername(req.body.username, function(err, user) {
    if (user) {
      ...
      var activityCouchConnection = {};

      activityCouchConnection = JSON.parse(JSON.stringify(couchConnection));
      activityCouchConnection.pouchname = req.body.username + "-activity_feed";

      var corpuses = [couchConnection];
      ...
      mostRecentIds.couchConnection.pouchname = JSON.parse(JSON.stringify(couchConnection.pouchname));
      ...
      /* Prepare a corpus doc for the user's first corpus */
      var corpus_template = JSON.parse(JSON.stringify(corpus_default_template));
      corpus_template.title = req.body.username + "\'s Corpus";
      corpus_template.titleAsUrl = corpus_template.title.toLowerCase().replace(" ", "_");
      corpus_template.team.username = req.body.username;
      corpus_template.couchConnection = couchConnection;
      corpus_template.pouchname = req.body.username + "-firstcorpus";
      ...
      var datalist_template = JSON.parse(JSON.stringify(datalist_default_template));
      datalist_template.pouchname = corpus_template.pouchname;
      datalist_template.dateCreated = JSON.parse(JSON.stringify(new Date()));
      datalist_template.dateModified = JSON.parse(JSON.stringify(new Date()));
      datalist_template.timestamp = Date.now();

      /* Prepare an empty session doc for the user's first corpus */
      var session_template = JSON.parse(JSON.stringify(session_default_template));
      session_template.pouchname = corpus_template.pouchname;
      session_template.dateCreated = JSON.parse(JSON.stringify(new Date()));
      session_template.dateModified = JSON.parse(JSON.stringify(new Date()));
      ...
      var team = {
        "_id": "team",
        "gravatar": "anonymousbydefault" + Date.now(),
        "username": "team",
        "collection": "users",
        "firstname": "",
        "lastname": "",
      ...
      var docsNeededForAProperFieldDBDatabase = {
        "user": user,
        "team" : team,
        "usersPublicSelfForThisCorpus": usersPublicSelfForThisCorpus,
        "userforcouchdb": userforcouchdb,
        "corpus" : corpus_template,
        "datalist": datalist_template,
      ...
      corpus_management.createDbaddUser(userforcouchdb.corpuses[0], userforcouchdb, function(res) {
        ...
        var connect = corpus_template.couchConnection.protocol + req.body.username + ":" + req.body.password + "@" + corpus_template.couchConnection.domain + ":" + corpus_template.couchConnection.port + "/" + corpus_template.pouchname;
        ...
        db.insert(corpus_template, function(err, body) {
          if (err) {
            ...
            undoCorpusCreation(user, corpus_template.couchConnection, docsNeededForAProperFieldDBDatabase);
          } else {
            console.log(new Date() + " Created corpus for " + corpus_template.pouchname + "\n");
            db.insert(datalist_template, function(err, body) {
              if (err) {
                console.log(new Date() + " There was an error in creating the datalist: " + util.inspect(err) + "\n");
                undoCorpusCreation(user, corpus_template.couchConnection, docsNeededForAProperFieldDBDatabase);
              } else {
                console.log(new Date() + " Created datalist for " + corpus_template.pouchname + "\n");
                db.insert(session_template, function(err, body) {
                  if (err) {
                    console.log(new Date() + " There was an error in creating the session: " + util.inspect(err) + "\n");
                    undoCorpusCreation(user, corpus_template.couchConnection, docsNeededForAProperFieldDBDatabase);
                  } else {
                    console.log(new Date() + " Created session for " + corpus_template.pouchname + "\n");
                    db.insert(team, function(err, body) {
                      if (err) {
                        console.log(new Date() + " There was an error in creating the team: " + util.inspect(err) + "\n");
                        undoCorpusCreation(user, corpus_template.couchConnection, docsNeededForAProperFieldDBDatabase);
                      } else {
                        console.log(new Date() + " Created team for " + corpus_template.pouchname + "\n");
                        db.insert(usersPublicSelfForThisCorpus, function(err, body) {
                          if (err) {
                            console.log(new Date() + " There was an error in creating the user's public self: " + util.inspect(err) + "\n");
                            undoCorpusCreation(user, corpus_template.couchConnection, docsNeededForAProperFieldDBDatabase);
                          } else {
                            console.log(new Date() + " Created users public self for " + corpus_template.pouchname + "\n");
...
var undoCorpusCreation = function(user, couchConnection, docs) {
  console.log("TODO need to clean up a broken corpus." + util.inspect(couchConnection));
  if(!user.email){
...
module.exports.fetchCorpusPermissions = function(req, done) {
  ...
  if (req.body.serverCode) {
    dbConn = corpus.getCouchConnectionFromServerCode();
  ...
  usersdb.view("users", "userroles", function(error, body) {
    if (error) {
       ...
       */
      var usersdb = nanoforpermissions.db
        .use(node_config.usersDbConnection.dbname);
...
function addCorpusToUser(error, username, newcorpusConnection, rolesresult,
  done) {
  if (error) {
    return done(error, null, {
      message: "There was a problem adding user to this corpus."
  ...
  findByUsername(
    ...
    function(err, user) {
      ...
      var indexOfNewCorpusInExistingCorpuses = _.pluck(user.corpuses,
        "pouchname").indexOf(newcorpusConnection.pouchname); // TODO test
      // this
       ...
       * email telling they they have access
       */
