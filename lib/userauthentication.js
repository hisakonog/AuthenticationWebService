 ...
 */
module.exports.registerNewUser = function(username, req, done) {
  if (username == "yourusernamegoeshere") {
  ...
  findByUsername(username, function(err, user) {
    if (user) {
      ...
      console.log(new Date() + ' Registering new user: ' + username);
      ...
      couchConnection.pouchname = "firstcorpus";
      var activityCouchConnection = {};

      activityCouchConnection = JSON.parse(JSON.stringify(couchConnection));
      activityCouchConnection.pouchname = username + "-activity_feed";
      ...
      var private_corpus_template = corpus.createPrivateCorpusDoc(username, username + "\'s Corpus", couchConnection, username + "-firstcorpus");

      /* Prepare a public corpus doc for the user's first corpus */
      var public_corpus_template = corpus.createPublicCorpusDoc(couchConnection, username + "-firstcorpus");

      /* Prepare an empty datalist doc for the user's first corpus */
      var datalist_template = corpus.createDatalistDoc(username + "-firstcorpus");

      /* Prepare an empty session doc for the user's first corpus */
      var session_template = corpus.createSessionDoc(username + "-firstcorpus");
      ...
      corpus_management.createDbaddUser(userforcouchdb.corpuses[0], userforcouchdb, function(res) {
        ...
        var connect = private_corpus_template.couchConnection.protocol + couch_keys.username + ":" + couch_keys.password + "@" + private_corpus_template.couchConnection.domain + ":" + private_corpus_template.couchConnection.port + "/" + private_corpus_template.pouchname;
