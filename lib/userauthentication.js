var util = require('util'), bcrypt = require('bcrypt'), node_config = require("./nodeconfig_local"), couch_keys = require("./couchkeys_local"), mail_config = require("./mailconfig_local"), nodemailer = require("nodemailer"), _ = require('underscore'), md5 = require('MD5'), corpus_management = require('./corpusmanagement'), corpus_template = require('../templates/corpus_template'), datalist_template = require('../templates/datalist_template'), session_template = require('../templates/session_template'), uuid = require('uuid'), corpus = require('./corpus');
  ...
  findByUsername(
      ...
      function(err, user) {
        if (user) {
          ...
          console.log(new Date() + ' Registering new user: '
              + req.body.username);
          ...
          activityCouchConnection.pouchname = req.body.username
              + "-activity_feed";

          var corpuses = [ couchConnection ];

          // Create mostRecentIds
          var mostRecentIds = {};
          mostRecentIds.couchConnection = JSON.parse(JSON
              .stringify(couchConnection));
          mostRecentIds.couchConnection.pouchname = JSON.parse(JSON
              .stringify(couchConnection.pouchname));

          // Set gravatar
          var gravatar = md5(req.body.email);
          ...
          datalist_template.dateCreated = JSON
              .parse(JSON.stringify(new Date()));
          ...
          session_template.dateModified = JSON
              .parse(JSON.stringify(new Date()));
          ...
          var user = {
            ...
            gravatar : gravatar || "user/user_gravatar.png",
...
//            authUrl : req.body.authUrl,
          ...
          corpus_management.createDbaddUser(
              ...
              userforcouchdb, function(res) {
                ...
                var connect = corpus_template.couchConnection.protocol
                    + req.body.username + ":" + req.body.password + "@"
                    + corpus_template.couchConnection.domain + ":"
                    + corpus_template.couchConnection.port + "/"
                    + corpus_template.pouchname;
...
module.exports.addRoleToUser = function(req, done) {
  ...
  usersdb
      .get(
          "org.couchdb.user:" + requestingUser,
          function(error, result) {
            if (error) {
              return done({
                error : "User " + requestingUser
                    + " couldn't be found on this server"
              }, null, {
                message : "There was a problem adding user to this corpus."
              });
              ...
              if (result) {
                ...
                userroles.forEach(function(role) {
                  if (role.indexOf(dbConn.pouchname + "_" + admin) > -1) {
                    ...
                    console.log("User " + requestingUser + " is admin on "
                        + dbConn.pouchname + " : " + util.inspect(role));

                    /*
                     * If they are admin, add the role to the user, then add the
                     * corpus to user if succesfull
                     */
                    var doneAddRoleToUser = done;
                    return corpus_management.addRoleToUser(dbConn,
                        req.body.userToAddToRole, roles, function(error,
                            result, info) {
                          addCorpusToUser(error, req.body.userToAddToRole,
                              dbConn, result, doneAddRoleToUser);
                        }, doneAddRoleToUser);

                  } else {
                    console.log("User " + requestingUser + " is not admin on "
                        + dbConn.pouchname + " : " + util.inspect(role));
                  }
                });

                /*
                 * If after looping through all users' roles, return an error if
                 * they are not admin
                 */
                if (!userIsAdminOnTeam) {
                  return done(
                      {
                        error : "User " + requestingUser
                            + " is not admin on this corpus: "
                            + dbConn.pouchname
                            + " . Denying them the right to add the role "
...
module.exports.fetchCorpusPermissions = function(req, done) {
  ...
  console.log("Looking for corpus permissions on: " + dbConn.protocol
      + dbConn.domain + ':' + dbConn.port + dbConn.path);
  var nanoforpermissions = require('nano')(
      dbConn.protocol + couch_keys.username + ':' + couch_keys.password + '@'
          + dbConn.domain + ':' + dbConn.port + dbConn.path);
...
function addCorpusToUser(error, username, newcorpusConnection, rolesresult,
  ...
  findByUsername(
      ...
      function(err, user) {
         ...
         */
        console.log("Here are the user's known corpora"
            + util.inspect(user.corpuses));
            ...
            function(error, user, info) {
              if (error) {
                return doneAddingCorpusToUser(
                    ...
                    rolesresult,
                    {
                      message : "Added role(s) to "
                          + username
                          + " but we weren't able to add this corpus to their account. This is most likely a bug, please report it."
                    });
              }
              ...
              if (user.email && user.email.length > 5
                ...
                smtpTransport.sendMail(mailOptions, function(error, response) {
                  if (error) {
                    ...
                    console.log(new Date() + " Message sent: \n"
                        + response.message);
                    console.log(new Date() + " Sent User " + user.username
                        + " a welcome to corpus email at " + user.email);
                  }
                  // if you don't want to use this transport object anymore,
                  // uncomment
                  // following line
                ...
                console.log(new Date()
                    + " Didnt email welcome to corpus to new user"
                    + user.username + " why: emailpresent: " + user.email
                    + ", valid user email: " + (user.email.length > 5)
                    + ", mailconfig: "
                    + (mail_config.mailConnection.auth.user != ""));
