...
module.exports.registerNewUser = function(localOrNot, req, done) {
  ...
  findByUsername(req.body.username, function(err, user) {
    if (user) {
      ...
      var private_corpus_template = corpus.createPrivateCorpusDoc(req.body.username, req.body.username + "\'s Corpus", couchConnection, req.body.username + "-firstcorpus");

      /* Prepare a public corpus doc for the user's first corpus */
      var public_corpus_template = corpus.createPublicCorpusDoc(couchConnection, req.body.username + "-firstcorpus");
      ...
      var docsNeededForAProperFieldDBDatabase = [user, team, usersPublicSelfForThisCorpus, userforcouchdb, public_corpus_template, private_corpus_template, datalist_template, session_template];
      ...
      corpus_management.createDbaddUser(userforcouchdb.corpuses[0], userforcouchdb, function(res) {
        ...
        var connect = private_corpus_template.couchConnection.protocol + req.body.username + ":" + req.body.password + "@" + private_corpus_template.couchConnection.domain + ":" + private_corpus_template.couchConnection.port + "/" + private_corpus_template.pouchname;
 ...
 */
module.exports.isRequestingUserAnAdminOnCorpus = function(req, requestingUser, dbConn, done){
  if (!dbConn) {
    return done({
  ...
  /* Catch all */
  return done({error: "User was found but didnt have permission."}, null, {info: "User does not have permission to perform this action."});
};
...
module.exports.addRoleToUser = function(req, done) {
  ...
  isRequestingUserAnAdminOnCorpus(req, requestingUser, dbConn, function(error, result, messages){
    if(error){
