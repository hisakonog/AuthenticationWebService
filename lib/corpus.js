...
var createNewCorpus = function(dbConnection, callback) {
  ...
  server.db.create(dbConnection.couchConnection.pouchname, function(err, body) {
    ...
    var corpus_template = createCorpusDoc(dbConnection.body.username, dbConnection.body.newCorpusName, dbConnection.couchConnection, dbConnection.couchConnection.pouchname);
...
var createCorpusDoc = function(username, corpusTitle, couchConnection, pouchname) {
  var newCorpus = JSON.parse(JSON.stringify(corpus_default_template));
  newCorpus.title = corpusTitle;
  newCorpus.titleAsUrl = newCorpus.title.toLowerCase().replace(" ", "_");
  // newCorpus.team.username = username;
  newCorpus.couchConnection = couchConnection;
  newCorpus.pouchname = pouchname;
  newCorpus.dateOfLastDatumModifiedToCheckForOldSession = JSON.parse(JSON.stringify(new Date()));
  newCorpus.confidential.secretkey = uuid.v4();
  // newCorpus.publicSelf.couchConnection = newCorpus.couchConnection;
  // newCorpus.publicSelf.pouchname = newCorpus.pouchname;
  return newCorpus;
};

module.exports.createCorpusDoc = createCorpusDoc;
