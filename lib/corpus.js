...
var updateRoles = function(dbConnection, callback) {
  ...
  if (dbConnection.body.userRoleInfo.removeUser
      && dbConnection.body.userRoleInfo.removeUser == true) {
    console.log("Removing permissions for " + username + " from "
        + dbConnection.couchConnection.pouchname);
  ...
  db.get(userid, function(err, body) {
    if (!err) {
      ...
      var corpusConnect = dbConnection.couchConnection.protocol
          + couch_keys.username + ":" + couch_keys.password + "@"
          + dbConnection.couchConnection.domain + ":"
          + dbConnection.couchConnection.port + "/"
          + dbConnection.couchConnection.pouchname;
      ...
      corpusDb.get(username, function(err, body) {
        if (!err) {
          ...
          // Add new user to DB
          // Get gravatar hash
          var zUserInfoDbConnect = dbConnection.couchConnection.protocol
              + couch_keys.username + ":" + couch_keys.password + "@"
              + dbConnection.couchConnection.domain + ":"
              + dbConnection.couchConnection.port + "/zfielddbuserscouch";
              ...
              db.insert(body, body._id, function(err, response) {
                if (!err) {
                  console.log("Successfully removed "
                      + dbConnection.couchConnection.pouchname
                      + " from org.couchdb.user:" + username);
                }
              });
