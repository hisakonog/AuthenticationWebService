...
module.exports.createDbaddUser = function(dbConnection, user, successcallback,
  ...
  console.log(new Date() + " Creating a new database/corpus: "
  ...
  if (useLocalCouchRatherThanRemoteCouch
    ...
    var c = new cradle.Connection(dbConnection.protocol + dbConnection.domain
        + dbConnection.path, dbConnection.port, {
      auth : {
        username : couch_keys.username,
        password : couch_keys.password
      }
  ...
  usersdb.save(userid, userParamsForNewUser, function(err, doc) {
    ...
    if (err != null || !doc.ok) {
      ...
      console.log(new Date() + " user " + userid
          + " created as a CouchDB user on: " + util.inspect(dbConnection));

      /*
       * Prepare the user's activity feed couch
       */
      createDBforUsersActivities(user.activityCouchConnection, user,
          function(res) {
            console.log(new Date()
                + " There was success in creating the users activity feed: "
                + res);
            ...
            createDBforCorpus(dbConnection, user, function(res) {
              console.log(new Date()
                  + " There was success in creating the users first corpus: "
                  + res);
              ...
              console.log(new Date()
                  + " There was an error in creating users first corpus: "
                  + err);
              if (typeof errorcallback == "function") {
                errorcallback();
              }
            ...
            console
                .log(new Date()
                    + " There was an error in creating users activity feed: "
                    + err);
            if (typeof errorcallback == "function") {
              errorcallback();
            }
...
var addRoleToUser = function(dbConnection, username, roles, successcallback,
  ...
  console.log(new Date() + " In addRoleToUser " + util.inspect(roles) + " to "
      + username + " on " + util.inspect(dbConnection));
...
var createDBforCorpus = function(dbConnection, user, success, error) {
  ...
  if (useLocalCouchRatherThanRemoteCouch
      && dbConnection.domain == node_config.usersDBExternalDomainName) {
    var c = new cradle.Connection();
    // console.log(new Date()+ " Using a local couchdb running on the same
    // server instead of "+node_config.usersDBExternalDomainName);
  } else {
    // console.log(new Date()+ " Using a remote couchdb instead of
    // "+node_config.usersDBExternalDomainName);
    var c = new cradle.Connection(dbConnection.protocol + dbConnection.domain
        + dbConnection.path, dbConnection.port, {
      auth : {
        username : couch_keys.username,
  ...
  db
      .exists(function(err, exists) {
        if (err) {
          ...
          console.log(new Date() + " The corpus " + dbConnection.pouchname
              + " exists, calling the errorcallback.");
                        ...
                        function() {
                          ...
                          createDBforCorpusTeamsActivities(
                              ...
                              function(res) {
                                console
                                    .log(new Date()
                                        + " There was success in creating the corpus team's activity feed: "
                                        + res);

                                if (typeof success == "function") {
                                  dbConnection.pouchname.replace(
                                      "-activity_feed", "");
                              ...
                              function(err) {
                                console
                                    .log(new Date()
                                        + " There was an error in creating the corpus team's activity feed: "
                                        + err);
                                if (typeof error == "function") {
                                  error(dbConnection);
                                }
...
var createDBforCorpusTeamsActivities = function(dbConnection, user,
  ...
  db.exists(function(err, exists) {
    if (err) {
      ...
      console.log(new Date()
          + " The users activity db exists, calling the errorcallback.");
      if (typeof errorcallback == "function") {
          ...
          .create(function(err, res) {
            ...
            var securityParamsforNewDB = {
              ...
              "members" : {
                "names" : [],
                "roles" : [
                    dbConnection.pouchname.replace("-activity_feed", "") + "_"
                        + collaborator,
                    dbConnection.pouchname.replace("-activity_feed", "") + "_"
                        + contributor ]
              }
...
module.exports.changeUsersPassword = function(dbConnection, user, newpassword,
  ...
  usersdb.get(userid, function(err, doc) {
    var userold = doc;
    console.log(new Date()
        + " These are the users's details before changing their password."
    ...
    usersdb.remove(userid, userold._rev, function(err, res) {
      ...
      usersdb.save(userid, userParamsForNewUser, function(err, doc) {
        ...
        if (err != null || !doc.ok) {
          ...
          console.log(new Date()
              + " User's couchdb password changed old revision number: "
              + userold._rev);
          if (typeof successcallback == "function") {
            successcallback(doc);
          }
