...
module.exports.createDbaddUser = function(dbConnection, user, successcallback,
  ...
  if (useLocalCouchRatherThanRemoteCouch && dbConnection.domain == node_config.usersDBExternalDomainName) {
    var c = new cradle.Connection();
  } else {

    var c = new cradle.Connection(dbConnection.protocol + dbConnection.domain + dbConnection.path, dbConnection.port, {
      auth: {
        username: couch_keys.username,
        password: couch_keys.password
      }
              ...
              .replicate({
                ...
                function() {
                  ...
                  createDBforCorpusTeamsActivities(
                    ...
                    function(res) {
                      console
                        .log(new Date() + " There was success in creating the corpus team's activity feed: " + res);
                    ...
                    function(err) {
                      console
                        .log(new Date() + " There was an error in creating the corpus team's activity feed: " + err);
                      if (typeof error == "function") {
                        error(dbConnection);
                      }
                    });
